...
class Game {
  ...
  // ---- Live Race: host-or-join lobby ------------------------------------

  _hostLive() {
    this.roomCode = genRoomCode();
    this._enterLobby(true);
  }
  ...
  _enterLobby(hosting) {
    ...
    this.state = 'lobby';
    this.hud.showLobby(this.roomCode);
  }
  ...
  _loop(now) {
    ...
    else if (this.state === 'lobby') this._updateLobby(dt);
  ...
  _updatePlaying(dt) {
    ...
    // Boost economy -- available in every mode.
